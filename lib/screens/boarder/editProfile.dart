...
/// Native picker boundary; selected bytes remain a draft until Save succeeds.
class ProfilePhotoPicker {
  ...
  Future<String?> recover() async {
    ...
    if (response.files?.isNotEmpty != true) return null;
    return _encode(response.files!.first);
  }

  Future<String> _encode(XFile file) async {
    // Store the normalized thumbnail itself, not a temporary camera/cache path.
