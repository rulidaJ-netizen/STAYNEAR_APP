  ...
  await tester.pumpWidget(
    MaterialApp(
      ...
      home: RepaintBoundary(
        key: capture,
        child: ScreenFrame(
          child: LandownerEditProfilePage(
            user: user,
...
void main() {
  ...
  for (final entry in {
    ...
    testWidgets('saving only ${entry.key} preserves the other fields', (
      tester,
    ) async {
      UserProfile? saved;
      await _show(tester, save: (user) async => saved = user);
      await _enter(tester, entry.key, entry.value);
      await _tap(tester, find.byKey(const ValueKey('save-profile')));
      expect(
        saved!.fullName,
        entry.key == 'profile-name' ? entry.value : _user.fullName,
      );
      expect(
        saved!.contact,
        entry.key == 'profile-phone' ? entry.value : _user.contact,
      );
      expect(
        saved!.email,
        entry.key == 'profile-email' ? entry.value : _user.email,
      );
      expect(
        saved!.address,
        entry.key == 'profile-address' ? entry.value : _user.address,
      );
      expect(saved!.profilePhoto, _user.profilePhoto);
      expect(saved!.role, UserRole.landlord);
    });
  }

  test(
    'an account cannot change its persisted role through Edit Profile',
    () async {
      final backend = TestFirebaseBackend();
      final user = await backend.signUpAndLogin(_user);
      final forged = FirebaseBackend.profileFromData(user.id, {
        ...FirebaseBackend.profileData(user),
        'role': 'boarder',
      });
      await expectLater(backend.saveProfile(forged), throwsStateError);
      expect((await backend.loadProfile()).role, UserRole.landlord);
    },
  );

  for (final scale in [1.0, 2.0]) {
    testWidgets(
      'Landowner Profile displays current data and working buttons at scale $scale',
      (tester) async {
        tester.view.physicalSize = const Size(320, 568);
        tester.view.devicePixelRatio = 1;
        addTearDown(tester.view.resetPhysicalSize);
        addTearDown(tester.view.resetDevicePixelRatio);
        var back = 0, edit = 0, logout = 0;
        await tester.pumpWidget(
          MaterialApp(
            home: MediaQuery(
        ...
        expect(find.text('Landowner'), findsOneWidget);
        expect(find.text('Boarder'), findsNothing);
        for (final value in [
          _user.fullName,
          _user.contact,
          _user.email,
        ...
        expect(
          tester
              .widget<LandownerProfileAvatar>(
                find.byType(LandownerProfileAvatar),
              )
              .source,
          _photo,
        );
    ...
    (tester) async {
      ...
      expect(
        await tester
            ...
            .onLogin(_user.email, _user.password, UserRole.landlord),
        isTrue,
      );
      await tester.pumpAndSettle();
      ...
      await tester.tap(find.byTooltip('Back'));
      await tester.pumpAndSettle();
      expect(find.byType(LandlordDashboard), findsOneWidget);
      tester
          .widget<LandlordDashboard>(find.byType(LandlordDashboard))
          .onProfile();
      await tester.pumpAndSettle();
      ...
      expect(find.text('Saved Maria'), findsOneWidget);
      ...
      await tester
          .widget<LandownerEditProfilePage>(
            find.byType(LandownerEditProfilePage),
          )
          .onSave(
      ...
      expect(
        tester
            .widget<LandownerProfileAvatar>(find.byType(LandownerProfileAvatar))
            .source,
        startsWith('https://'),
      );
      expect(
      ...
      expect(
        await auth.onLogin(_user.email, _user.password, UserRole.landlord),
        isTrue,
      );
      await tester.pumpAndSettle();
      expect(find.byType(LandlordDashboard), findsOneWidget);
  ...
  testWidgets('missing and unreadable avatar sources fall back safely', (
    ...
    for (final source in [
      ...
      expect(find.byType(LandownerProfileAvatar), findsOneWidget);
      expect(tester.takeException(), isNull, reason: source);
    }
  });

  testWidgets('optional Edit Profile visual preview', (tester) async {
    if (!const bool.fromEnvironment('LANDOWNER_EDIT_PROFILE_PREVIEW')) return;
    ...
    await tester.runAsync(() async {
      ...
      await File('build/landowner-edit-profile-preview.png')
