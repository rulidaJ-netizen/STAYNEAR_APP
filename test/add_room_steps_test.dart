...
const photo = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAJcEhZcwAADsMAAA7DAcdvqGQAAAANSURBVBhXY2Bg+P8fAAMCAf/Jsq3uAAAAAElFTkSuQmCC';
class Picker extends ProfilePhotoPicker {
  ...
  Future<String?> pick(ImageSource source) async {
    ...
    return pending?.future ?? Future.value(photo.replaceFirst('image/png;', 'image/png;name=${count++};'));
  }
}
...
void fill(WidgetTester tester, String hint, String value, {int index = 0}) {
  tester.widgetList<TextField>(find.byType(TextField)).where((field) => field.decoration?.hintText == hint).elementAt(index).controller!.text = value;
}

Future<void> show(WidgetTester tester, Picker picker, {int initial = 0, FutureOr<void> Function(Listing)? publish, VoidCallback? published, double scale = 1, Size size = const Size(390, 985), GlobalKey? capture}) async {
  ...
  await tester.pumpWidget(MaterialApp(home: MediaQuery(
    ...
    child: RepaintBoundary(key: capture, child: ScreenFrame(child: StatefulBuilder(builder: (context, update) => RoomWizard(
      step: step, photoPicker: picker, onBack: () {}, onProfile: () {},
      onNext: () => update(() => step++), onPrevious: () => update(() => step--), onPublish: publish ?? (_) {}, onPublished: published,
    )))),
  )));
  await tester.pumpAndSettle();
}
...
void main() {
  testWidgets('four photo slots validate, preview, remove, retain, and publish every field', (tester) async {
    Listing? result;
    final picker = Picker();
    await show(tester, picker, publish: (listing) => result = listing);
    fill(tester, 'Cozy Student Room near Campus', 'Test property');
    fill(tester, 'Describe the room, facilities, and nearby amenities...', 'Description');
    fill(tester, '09171234567', '09991234567');
    await tap(tester, 'Next');
    await tap(tester, 'Next');
    expect(find.text('Please select at least one property photo.'), findsOneWidget);
    ...
    fill(tester, 'Paste a Google Maps link', 'https://maps.app.goo.gl/testProperty');
    await tap(tester, 'Previous');
    expect(tester.widget<ChoiceChip>(find.widgetWithText(ChoiceChip, 'Parking')).selected, isTrue);
    await tap(tester, 'Next');
    await tap(tester, 'Publish Listing');
    ...
    expect(result!.houseInformation['Reference Map'], 'https://maps.app.goo.gl/testProperty');
    expect(result!.houseInformation['Distance from University'], '800 m');
    expect(tester.takeException(), isNull);
  });

  testWidgets('picker cancel, permission error and late completion preserve drafts safely', (tester) async {
    final picker = Picker()..cancel = true;
    await show(tester, picker, initial: 1);
    await tap(tester, 'Photo 1'); await tap(tester, 'Choose from Gallery');
    expect(find.text('Selected: 0 / 4'), findsOneWidget);
    picker..cancel = false..fail = true;
    await tap(tester, 'Photo 1'); await tap(tester, 'Take Photo');
    expect(find.textContaining('Check photo permissions'), findsOneWidget);
    picker..fail = false..pending = Completer<String?>();
  ...
  testWidgets('publish waits for success, prevents duplicates, and reports failure', (tester) async {
    ...
    fill(tester, 'Describe the room, facilities, and nearby amenities...', 'Retry description');
    fill(tester, '09171234567', '09991234567');
    await tap(tester, 'Next');
    await tap(tester, 'Photo 1');
    await tap(tester, 'Choose from Gallery');
    ...
    expect(find.byType(Dialog), findsNothing);
    expect(
      find.text('simulated database failure'),
      findsOneWidget,
    );
  ...
  test('Google Maps accepts full and short links without accepting unrelated hosts', () {
    for (final link in ['https://www.google.com/maps?q=9.95,124.03', 'https://maps.google.com/?q=Clarin', 'https://maps.app.goo.gl/abc', 'https://goo.gl/maps/abc']) {
      expect(PropertyLocationService.savedMapUri(link).toString(), link);
    }
    for (final link in ['', 'javascript:alert(1)', 'https://google.com.evil.test/maps', 'https://example.test/maps']) {
      expect(PropertyLocationService.savedMapUri(link), isNull);
    }
  });

  for (final step in [1, 2, 3]) {
    testWidgets('step ${step + 1} fits small screens with large text and keyboard', (tester) async {
      await show(tester, Picker(), initial: step, scale: 2, size: const Size(320, 568));
      tester.view.viewInsets = const FakeViewPadding(bottom: 220);
      addTearDown(tester.view.resetViewInsets);
      await tester.pumpAndSettle();
      await tester.ensureVisible(find.text(step == 3 ? 'Publish Listing' : 'Next'));
      await tester.pumpAndSettle();
      expect(tester.takeException(), isNull);
    });
  }
  ...
  testWidgets('optional steps visual previews', (tester) async {
    ...
    await tester.runAsync(() async {
      ...
      for (final entry in {'Roboto': ['roboto-regular.ttf', 'roboto-medium.ttf', 'roboto-bold.ttf'], 'MaterialIcons': ['materialicons-regular.otf']}.entries) {
        final loader = FontLoader(entry.key);
        for (final file in entry.value) { loader.addFont(File('$sdk/bin/cache/artifacts/material_fonts/$file').readAsBytes().then(ByteData.sublistView)); }
        await loader.load();
      }
    });
    for (final step in [1, 2, 3]) {
      ...
      await show(tester, Picker(), initial: step, size: Size(390, step == 1 ? 985 : 930), capture: capture);
      await tester.runAsync(() async {
        final image = await (capture.currentContext!.findRenderObject()! as RenderRepaintBoundary).toImage(pixelRatio: 2);
        final bytes = await image.toByteData(format: ui.ImageByteFormat.png);
        await File('build/room-step-${step + 1}.png').writeAsBytes(bytes!.buffer.asUint8List()); image.dispose();
      });
    }
  });
}
